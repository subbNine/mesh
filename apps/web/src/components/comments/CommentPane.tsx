...
const formatTime = (dateStr: string) => {
  ...
  if (diffInMinutes < 60) return diffInMinutes <= 0 ? 'Just now' : `${diffInMinutes}m ago`;
...
export function CommentPane({ taskId, ydoc, currentUser, activeCommentId, onCommentClick }: CommentPaneProps) {
  ...
  const fetchComments = async (syncYdoc = false) => {
    try {
      const res = await api.get(`/tasks/${taskId}/comments`);
      const fetched: CanvasComment[] = res.data;
      setComments(fetched);
      
      // Dual-storage sync mechanism: Push API state to Yjs if requested
      if (syncYdoc) {
        ydoc.transact(() => {
          const yComments = ydoc.getArray<Y.Map<any>>('comments');
          // Since it's a collaborative doc, we only push what doesn't exist to prevent duplicates
          ...
          for (const c of fetched) {
            if (!existingIds.has(c.id)) {
              ...
              map.set('initials', getInitials(c.author.firstName, c.author.lastName));
              map.set('color', getUserColor(c.author.id));
              map.set('replyCount', c.replies.length);
              map.set('resolvedAt', c.resolvedAt);
              yComments.push([map]);
            } else {
              // Update reply counts & resolved status of existing elements
  ...
  useEffect(() => {
    ...
    // Listen for incoming Yjs comments that we don't have yet
    const handleYUpdate = () => {
      fetchComments(false);
    };
    const yComments = ydoc.getArray<Y.Map<any>>('comments');
  ...
  const handleResolve = async (e: React.MouseEvent, id: string, isResolved: boolean) => {
    ...
    try {
      ...
      ydoc.transact(() => {
        ...
        if (target) {
          target.set('resolvedAt', isResolved ? null : new Date().toISOString());
        }
      });
    } catch(e) {}
  };
          ...
          return (
            <div
              ...
              className={`border rounded-xl overflow-hidden transition-all duration-200 cursor-pointer 
                ...
                ${isActive 
                  ? 'border-primary ring-2 ring-primary/10 shadow-md bg-white' 
                  : 'border-zinc-200/80 shadow-sm hover:border-zinc-300 bg-white'}`}
            >
              {/* Root comment */}
              <div className="p-4">
                ...
                <div className="ml-8 mt-2 text-[10px] uppercase font-bold tracking-widest text-zinc-400 flex items-center gap-1">
                  <span>📍 {Math.round(comment.canvasX)}, {Math.round(comment.canvasY)}</span>
                </div>
              </div>

              {/* Replies Thread */}
              {(comment.replies.length > 0 || isActive) && (
                <div className="border-t border-zinc-100 bg-zinc-50/50">
                  {!isResolved && comment.replies.map((reply) => (
                    <div key={reply.id} className="p-3 pl-12 border-b border-zinc-100/50 last:border-b-0">
                      <div className="flex items-start justify-between gap-2 mb-1.5">
                        ...
                        <span className="text-[10px] text-zinc-400 font-medium">{formatTime(reply.createdAt)}</span>
                      </div>
                    ...
                    <div className="p-3 pl-12 bg-zinc-50/50" onClick={e => e.stopPropagation()}>
                      <div className="flex items-end gap-2 bg-white border border-zinc-200 rounded-xl px-3 py-2 focus-within:border-primary focus-within:ring-2 focus-within:ring-primary/10 transition-all shadow-sm">
                        ...
                        <button
                          ...
                          className="w-[26px] h-[26px] bg-primary hover:bg-primary-600 disabled:opacity-40 disabled:cursor-not-allowed text-white rounded-md flex items-center justify-center transition-colors flex-shrink-0 mb-[-2px]"
                        >
